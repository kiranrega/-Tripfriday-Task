...
const UserPosts = () => {
  ...
  const { value: newComment, handleChange: handleCommentChange, resetValue: resetNewComment } = useFormInput('');
  ...
  <div className="flex justify-between items-center mb-4 cursor-pointer" onClick={() => setShowPosts(!showPosts)}>
    <h2 className="text-xl font-bold text-gray-800">{showPosts ? 'Hide Posts' : 'Show Posts'}</h2>
    <span className={`transform transition-transform ${showPosts ? 'rotate-180' : ''}`}>
    ...
    <div className="mt-4">
      ...
      <div className="mt-4">
            <ReactPaginate
              ...
              pageClassName={'px-3 py-1 border border-gray-300 hover:bg-gray-200 cursor-pointer'}
            />
          </div>
    </div>
  )}
